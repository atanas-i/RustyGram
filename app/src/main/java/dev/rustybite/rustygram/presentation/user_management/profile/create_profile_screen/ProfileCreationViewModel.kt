                ...
                getFileFromUri(uri, resProvider, "").collectLatest { result ->
                    when(result) {
                        is RustyResult.Success -> {
                            ...
                            storageRepository.uploadProfilePicture(file, "user_id_one", file.name).collectLatest { uploadResult ->
                                when(uploadResult) {
                                    is RustyResult.Success -> {
                                        _uiState.value = _uiState.value.copy(
                                    ...
                                    is RustyResult.Failure -> {
                                        _uiState.value = _uiState.value.copy(
                                            errorMessage = uploadResult.message ?: resProvider.getString(R.string.unknown_error),
                                            loading = false
                                        )
                                    }
                                    is RustyResult.Loading -> {
